...
pub(in crate::lower) fn add_native_fns(ctxt: &mut Ctxt) {
    ...
    for (fn_ident, generator) in NATIVE_FNS.iter() {
        ...
        let call = ctxt.push_compute(hir::Expr::Function(fn_id));
...
fn print_native_fn(ctxt: &mut Ctxt, _native_impls: &NativeImpls) {
    ...
    let args = ctxt.push_compute(hir::Expr::Index(arg, ctxt.args_str()));
    let arg1 = ctxt.push_compute(hir::Expr::Index(args, ctxt.one()));
    ...
    ctxt.push_st(hir::Statement::Print(call_node));
    ctxt.push_goto(post_bid);

    ctxt.set_active_block(else_bid);
    ctxt.push_st(hir::Statement::Print(arg1));
    ctxt.push_goto(post_bid);
...
fn type_native_fn(ctxt: &mut Ctxt, _native_impls: &NativeImpls) {
    ...
    let args = ctxt.push_compute(hir::Expr::Index(arg, ctxt.args_str()));
    let arg1 = ctxt.push_compute(hir::Expr::Index(args, ctxt.one()));
    let val = ctxt.push_compute(hir::Expr::Type(arg1));
...
fn next_native_fn(ctxt: &mut Ctxt, _native_impls: &NativeImpls) {
    ...
    let arg2 = ctxt.push_compute(hir::Expr::Index(args, two));
    let new_index = ctxt.push_compute(hir::Expr::Next(arg1, arg2));
    let new_val = ctxt.push_compute(hir::Expr::Index(arg1, new_index));
...
fn pairs_native_fn(ctxt: &mut Ctxt, native_impls: &NativeImpls) {
    ...
    let next_fn = ctxt.push_compute(hir::Expr::Function(native_impls["next"]));
    ...
    let nil_node = ctxt.push_compute(hir::Expr::Nil);
